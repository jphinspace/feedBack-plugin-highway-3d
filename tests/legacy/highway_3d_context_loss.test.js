...
test('binds webglcontextlost + webglcontextrestored on the renderer canvas', () => {
  assert.match(
    domAndSceneSrc,
    /ren\.domElement\.addEventListener\(\s*['"]webglcontextlost['"]/,
    'must listen for webglcontextlost on ren.domElement (the WebGL canvas)',
  );
  assert.match(
    domAndSceneSrc,
    /ren\.domElement\.addEventListener\(\s*['"]webglcontextrestored['"]/,
...
test('draw() early-returns while the context is lost', () => {
  assert.match(
    src,
    /draw\(bundle\)\s*\{[\s\S]*?if\s*\(_ctxLost\)\s*return;/,
...
test('teardown removes the context-loss listeners', () => {
  assert.match(
    src,
    /removeEventListener\(\s*['"]webglcontextlost['"]/,
    'teardown must remove the webglcontextlost listener',
  );
  assert.match(
    src,
    /removeEventListener\(\s*['"]webglcontextrestored['"]/,
