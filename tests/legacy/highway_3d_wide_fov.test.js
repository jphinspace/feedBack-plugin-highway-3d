...
test('effectiveVfov returns the base fov when the bridge is off/absent', async () => {
  ...
  assert.equal(
    effectiveVfov(32 / 9, { enabled: false, baseVfov: 65 }),
    65,
...
test('a floating tuner panel is built and can be shown/hidden', () => {
  assert.match(
    panelSrc,
    /function\s+_ensureAspectPanel\s*\(\)/,
    '_ensureAspectPanel() must exist to build the live panel',
  );
  assert.match(
    panelSrc,
    /function\s+_setAspectPanelVisible\s*\(/,
...
test('a Target select and pane registry drive the per-pane picker', () => {
  assert.match(
    panelSrc,
    /_aspectTargetSel\s*=\s*document\.createElement\(\s*'select'\s*\)/,
    'the panel must build a Target <select>',
  );
  assert.match(
    panelSrc,
    /function\s+_aspectRegisterPane\s*\(/,
...
test('panes are keyed by arrangement (stable across songs, no split-API dep)', () => {
  ...
  assert.match(
    panelSrc,
    /function\s+_aspectPaneKey\s*\(\s*arrangement\s*,\s*uid\s*\)[\s\S]*?arr:\$\{a\}[\s\S]*?pane:\$\{uid\}/,
    '_aspectPaneKey must prefer arr:<name> and fall back to pane:<uid>',
  );
  assert.match(
    cameraLifecycleSrc,
    /_arrangement\s*=\s*bundle[\s\S]*?songInfo[\s\S]*?arrangement[\s\S]*?_aspectPaneKey\(\s*_arrangement\s*,\s*_paneUid\s*\)/,
...
test('the target dropdown prunes dead panes and does not rebuild while focused', () => {
  assert.match(
    panelSrc,
    /function\s+_aspectPrunePanes\s*\(\)[\s\S]*?delete\s+reg\[k\]/,
    '_aspectPrunePanes must drop panes not seen recently',
  );
  assert.match(
    panelSrc,
    /_aspectPrunePanes\(\)\s*;[\s\S]*?if\s*\(\s*_aspectPanesDirty\s*\)\s*_aspectBuildTargets\(\)/,
    'the readout tick must prune then rebuild only when dirty',
  );
  assert.match(
    panelSrc,
    /function\s+_aspectBuildTargets\s*\(\)[\s\S]*?document\.activeElement\s*===\s*_aspectTargetSel[\s\S]*?return/,
...
test('programmatic sync does not write back into the tune', () => {
  ...
  assert.match(
    panelSrc,
    /_aspectSyncing\s*=\s*true[\s\S]*?finally[\s\S]*?_aspectSyncing\s*=\s*false/,
    '_syncAspectPanel must set/reset the _aspectSyncing guard',
  );
  assert.match(
    panelSrc,
    /if\s*\(\s*!_aspectSyncing\s*\)\s*_aspectWriteVal\(\s*f\.k\s*,/,
...
test('unchecking hfov override clears a pane override key (re-inherits base)', () => {
  ...
  assert.match(
    panelSrc,
    /else\s+_aspectClearVal\(\s*'hfovDeg'\s*\)/,
...
test('Reset on All restores defaults exactly (no forced enabled)', () => {
  ...
  assert.doesNotMatch(
    panelSrc,
    /Object\.keys\(_ASPECT_DEFAULTS\)[\s\S]*?base\.enabled\s*=\s*true/,
...
test('the shortcut opens/closes the tuner panel', () => {
  ...
  assert.match(
    panelSrc,
    /function\s+_toggleAspectPanel\s*\(\)/,
