...
test('cam.position uses interpolated framing multipliers, not literals', () => {
  ...
  assert.match(
    cameraLifecycleSrc,
    /_camX\s*=\s*ctx\.cam\.curX\s*\+\s*shoulderOffset\s*;[\s\S]*?_camY\s*=\s*h\s*\*\s*_hMul\s*;[\s\S]*?_camZ\s*=\s*dist\s*\*\s*_dMul/,
...
test('framing multipliers are a clamped zoom-distance interpolation', () => {
  ...
  assert.match(
    cameraLifecycleSrc,
    /Math\.max\(0,\s*Math\.min\(\s*1,[\s\S]*?CAM_FRAME_DIST_NEAR[\s\S]*?CAM_FRAME_DIST_FAR/,
