...
// runtime via window.h3dDevSetFretSpacing and persisted in localStorage. A
...
// rather than regexing its declaration. window.h3dDevSetFretSpacing itself
...
test('h3dDevSetFretSpacing validates the mode against the two supported values', () => {
...
test('h3dDevSetFretSpacing applies the change live, not via a page reload', () => {
  ...
  const src = fs.readFileSync(SCREEN_JS, 'utf8');
  const setter = src.match(/window\.h3dDevSetFretSpacing\s*=\s*\(mode\)\s*=>\s*\{[\s\S]*?\n\};/);
  assert.ok(setter, 'h3dDevSetFretSpacing assignment must be present');
  assert.doesNotMatch(
    setter[0],
    /location\.reload/,
    'h3dDevSetFretSpacing must not reload the page (it ejects the user from Settings)',
  );
  assert.match(
    setter[0],
    /emitSettingChange\(\s*'fretSpacing'\s*\)/,
    'h3dDevSetFretSpacing must broadcast a live fretSpacing change',
  );
});
