...
test('applySize pins the .h3d-wrap overlay to the highway canvas rect box', () => {
  ...
  assert.match(fn, /wrap\.style\.top\s*=\s*[`'"$({]*_cr\.top\s*-\s*_pr\.top\s*-\s*_pbTop/, 'top must be canvas rect relative to the containing block padding edge');
  assert.match(fn, /wrap\.style\.left\s*=\s*[`'"$({]*_cr\.left\s*-\s*_pr\.left\s*-\s*_pbLeft/, 'left must be canvas rect relative to the containing block padding edge');
...
test('applySize fallback resets the static anchor and the computed height', () => {
  ...
  assert.match(fallback, /wrap\.style\.height\s*=\s*(?:h\s*\+\s*['"]px['"]|`\$\{h\}px`)/, 'fallback must keep the computed height');
});
