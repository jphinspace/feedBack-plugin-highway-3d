...
test('a gaussian DataTexture helper (_makeGaussTex) drives the bloom falloff', async () => {
    // _makeGaussTex itself moved to src/core/texture.js in the screen.js -> src/
    // module split (Stage 1); real-import it rather than regexing its
    // declaration. The call site (_bloomGaussTex = _makeGaussTex(...)) moved
    // to instance/geometry/sustain-rail.js in Stage 7 Track A.
    const { _makeGaussTex } = await import('../../src/core/texture.js');
