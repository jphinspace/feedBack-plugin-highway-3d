...
import {
  settingsMemFallback, settingsListeners, emitSettingChange, readGlobalSetting,
...
test('intensity writes once on release, not on every drag step', async () => {
  ...
  assert.equal(
    writes.filter((w) => w[0] === 'intensity').length,
    0,
