...
- **Two naming worlds, and the boundary is load-bearing.** Module internals are named plainly (`readSetting`, `mountBackgroundStyle`, `createButterchurnController`). Three surfaces are *frozen* and keep their historical `Bg`/`Bc` spelling — renaming them breaks things silently, outside this repo:
  ...
  - **Existing `localStorage` keys** (`h3d_bg_*`, `viz3d_*`, `highway_3d.fretSpacing`, `h3d_aspect_tune2`) remain shared for A/B preference parity. The exception is `h3d_dev_bg_customVideoName`: its filename must follow this plugin's independent `plugin_uploads/highway_3d_dev/` file slot.
...
- **Plugin id vs. the bundled plugin.** `plugin.json`'s `id` is `highway_3d_dev` (name `"3D Highway (dev)"`) — deliberately *not* `highway_3d`, the id of the plugin bundled in-tree with feedBack core. Core's plugin loader silently discards a user-installed plugin whose `id` collides with a bundled one's (see `README.md`), so a same-id fork would never actually load. The distinct id lets this fork install side-by-side with the real bundled plugin, with a namespaced factory, mutable browser API, routes, and upload slot. Most visual preferences stay shared to make A/B comparison useful; server-backed custom video is isolated because the two plugins own different files.
...
- **Settings + favorites/bans** → [bc/prefs.js](src/butterchurn/prefs.js) (`viz3d_settings`, `viz3d_favorites`, `viz3d_banned`, `viz3d_seeded`). `window.h3dDevBcApplySettings` (in main.js) drops the cache via `resetButterchurnSettingsCache()` and re-applies to every live controller.
