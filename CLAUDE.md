...
- **`src/globals.js` is the ONLY writer of the plugin's *public* `window.*` contract** — every binding spelled out longhand, one line each, so `settings.html` and external callers stay greppable. Don't loop over an object to assign them. **Exception, deliberately outside this rule**: a handful of double-underscore-prefixed globals are private cross-module-instance coordination channels, not part of the public contract, and are written directly by their owning module: `window.__h3dAspectTune`/`__h3dAspectPanes`/`__h3dAspectPanelOpen`/`__h3dAspectReadout` (owned by `ui/aspect-panel.js`, read by `instance/render/camera-lifecycle.js` — documented at the top of `aspect-panel.js`) and `window.__feedBackAudioTap` (owned by `audio/analyser.js`) exist specifically because `window.*` is the only channel that survives the reload-cache-buster's per-generation module-instance multiplication (a fresh reload gets a *new* ES module graph, so a plain module-scope variable wouldn't be seen by an older-generation instance) and, for the audio tap, because it's shared across *plugin* boundaries (another visualizer's separate module graph entirely) — neither case can be expressed as an `installGlobals()` one-time binding since the values are lazily created and mutated throughout the session, not stable function references assigned once at boot.
...
- **Plugin id vs. the bundled plugin.** `plugin.json`'s `id` is `highway_3d_dev` (name `"3D Highway (dev)"`) — deliberately *not* `highway_3d`, the id of the plugin bundled in-tree with feedBack core. Core's plugin loader silently discards a user-installed plugin whose `id` collides with a bundled one's (see `README.md`), so a same-id fork would never actually load — this repo used to work around that by symlinking itself *over* the bundled copy (see `docs/dev-loop.md`'s git history), which meant only one of the two could ever be active. The distinct id lets this fork install side-by-side with the real bundled plugin instead, showing up as its own entry in the visualizer picker/plugin manager/settings UI, without masking or replacing the bundled one. This changes every `id`-derived surface — the registered global (`window.feedBackViz_highway_3d_dev`, not `_highway_3d`), `routes.py`'s `PLUGIN_ID` and its `/api/plugins/highway_3d_dev/...` routes, the `plugin_uploads/highway_3d_dev/` upload dir, and `plugin.json`'s own `"bundled": false` (this fork isn't the bundled copy, so it shouldn't claim the "Bundled" UI badge) — but deliberately does **not** touch anything in the frozen-naming-surface list above: every `localStorage` key (including `highway_3d.fretSpacing`, despite the name) and every `window.h3dBgSet*`/`h3dVenue*`/`h3dBcApplySettings` function stay exactly as-is, so this dev build reads/writes the *same* saved settings as the bundled plugin rather than starting from defaults.
