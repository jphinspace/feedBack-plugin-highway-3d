 ...
 */
const VENDOR_BASE_URL = '/api/plugins/highway_3d_dev/assets/vendor/';
const PCM_FRAME_SIZE = 1024;
const PCM_WORKLET_URL = '/api/plugins/highway_3d_dev/assets/viz-worklet.js';
...
export function loadButterchurnLib() {
  ...
  libLoadPromise = new Promise((resolve, reject) => {
    ...
    add(`${VENDOR_BASE_URL}butterchurn.min.js`, () => add(`${VENDOR_BASE_URL}butterchurnPresets.min.js`, resolve));
  });
...
export function createGuitarPcmFeed(actx, onReady) {
  ...
  let polling = true; let songLevel = 0; let
    chartLevel = 0;
  let node = null; let sp = null; let
    silent = null;
  ...
  function useScriptProcessor() {
    ...
    sp.onaudioprocess = (e) => {
      ...
      const lvl = songLevel; const clvl = chartLevel; const
        gg = gainNow();
      ...
      if (f && f.length) {
        ...
        if (node) {
          node.port.postMessage({
            frame: latest.slice(0), song: songLevel, chart: chartLevel, gain: gainNow(),
          });
        }
      }
