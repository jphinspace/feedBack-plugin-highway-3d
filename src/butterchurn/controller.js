import {
  createGuitarPcmFeed, isDesktopAudioHost, loadButterchurnLib, resolvePresetsGlobal, releaseCanvasGL, resolveButterchurnGlobal,
} from './engine.js';
import {
  bannedPresets, butterchurnControllers, favoritePresets, loadPresetLists, loadButterchurnSettings, savePresetLists,
} from './prefs.js';
import {
  ensurePresetPanel, primaryController, setPrimaryController, teardownPresetPanel, updatePanelPreset,
} from './panel.js';
...
export function createButterchurnController(wrap, sizeProvider, audioProvider) {
  const ctrl = {
    viz: null, actx: null, guitar: null, map: null, keys: [], cycle: 0, dead: false, lastW: -1, lastH: -1, canvas: null, backdrop: null, scrim: null, tint: null, wrap,
  };
  ...
  ctrl.applySettings = function applySettings() {
  ...
  loadButterchurnLib().then(() => {
    ...
    if (isDesktopAudioHost()) {
      ...
      try { ctrl.viz.connectAudio(fogAudio.analyser); console.log(`[viz3d] browser: Butterchurn tapping shared analyser (${fogAudio.source || 'core'})`); } catch (e) { console.warn('[viz3d] shared-analyser connect failed', e); }
    }
    loadPresetLists();
  ...
  function applyButterchurnSize(cssW, cssH) {
    ...
    const bw = Math.max(1, Math.round(cssW * ratio)); const
      bh = Math.max(1, Math.round(cssH * ratio));
  ...
  return {
    ...
    tint(hex, alpha) {
      ...
      const r = (hex >> 16) & 255; const g = (hex >> 8) & 255; const
        b = hex & 255;
