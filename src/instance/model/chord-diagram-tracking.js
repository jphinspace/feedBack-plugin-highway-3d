...
export function updateChordDiagramTracking(
  ...
  if (chords) {
    ...
    for (let _di = _dlo; _di < chords.length; _di++) {
      ...
      if (lbl && tmpl?.frets) {
        newChord = {
          name: lbl, frets: tmpl.frets, t: ch.t, t0: ch.t, chDt, nStr,
        };
      }
    }
  }
  ...
  if (newKey !== diagLastKey) {
    if (diagChord && newKey !== null) {
      ...
      if (freshChDt <= 0 && prevOpacity > 0) {
        ...
        diagPrev = {
          name: diagChord.name, frets: diagChord.frets, nStr: diagChord.nStr ?? nStr, t: diagChord.t0 ?? diagChord.t ?? now,
        };
        ...
        if (chords && newChord) {
          ...
          for (let _hpi = 0; _hpi < _hpHi; _hpi++) {
            ...
            if (lbl && tmpl?.frets) {
              histPrev = {
                name: lbl, frets: tmpl.frets, t: ch.t, t0: ch.t, nStr,
              };
            }
          }
        }
        ...
        if (histPrevVisible && elapsed >= 0 && elapsed < DIAG_CROSSFADE_S) {
          ...
          const histStartOpacity = Math.max(0, Math.min(
            1,
