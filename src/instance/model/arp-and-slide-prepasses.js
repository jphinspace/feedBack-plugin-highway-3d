...
export function createArpAndSlidePrepasses({ chordInference, arpeggioLaneRail, _scrArpPersistKeys }) {
  const _arpLaneRailHsScratch = [];
  const _arpRailBoundLoScratch = [];
  const _arpRailBoundHiScratch = [];
  ...
  const _arpSynthOnsetHsSet = new Set();
  ...
  function computeArpPersistKeys(arpGhostHsInfer, handShapes, notes, now, t0) {
    ...
    if (arpGhostHsInfer && handShapes && notes) {
      for (let _hi = 0; _hi < handShapes.length; _hi++) {
        ...
        const _lo = chordInference.hsStart(_hs); const
          _hi2 = chordInference.hsEnd(_hs);
  ...
  return {
    computeArpPersistKeys,
    computeSlideTargetSet,
    computeLaneRailCaches,
    computeMergedChords,
    resetMergeCache,
