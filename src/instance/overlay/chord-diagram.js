...
export function createChordDiagramCache() {
  ...
  function drawDiagramCached(ctx, opts) {
    const {
      opacity = 1, entranceT = 1.0, canvasW, canvasH,
    } = opts;
    ...
    const {
      name, frets, nStr, inverted, sizeSlider, position, lyricsBottom = 0, stackOffset = 0,
    } = opts;
    ...
    if (!entry) {
      let oc;
      try { oc = new OffscreenCanvas(canvasW, canvasH); } catch (_) { oc = document.createElement('canvas'); oc.width = canvasW; oc.height = canvasH; }
