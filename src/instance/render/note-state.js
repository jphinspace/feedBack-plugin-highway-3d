...
export function createFrameState({
  ...
  function buildFrameState(notes, chords, now, nStr) {
    ...
    if (notes) {
      ...
      for (let _ni = _fsLo; _ni < notes.length; _ni++) {
        ...
        if (dt > 0 && dt < 0.6) noteState.stringAnticipation[n.s] = Math.max(noteState.stringAnticipation[n.s], 1 - dt / 0.6);
        if (n.f > 0) {
    ...
    if (chords) {
      ...
      for (let _cni = _cfsLo; _cni < chords.length; _cni++) {
        ...
        for (const cn of chordNotes) {
          if (dt > 0 && dt < 0.6) noteState.stringAnticipation[cn.s] = Math.max(noteState.stringAnticipation[cn.s], 1 - dt / 0.6);
          if (cn.f > 0) {
        ...
        if (now >= ch.t && now <= susEnd) for (const cn of chordNotes) noteState.stringSustain[cn.s] = true;
  ...
  function updateStringHighlights(noteState, nStr, glowMul, _vibrancyIdleOp) {
    ...
    for (let s = 0; s < nStr; s++) {
      ...
      if (mAccentCore[s]) {
        mAccentCore[s].emissiveIntensity = (bg + noteState.accentFillBoost[s] * g) * venueGemMul;
      }
    }
  }
