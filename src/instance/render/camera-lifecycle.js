...
export function createCameraLifecycle({
  ...
  let _appliedW = 0;
  let _appliedH = 0;
  // _aspectPaneKey() builds a string; the arrangement it's keyed on only changes on song
  // load, so cache it here instead of re-allocating a string every frame.
  let _cachedPaneKeyArrangement;
  let _cachedPaneKey;
  ...
  function camUpdate(bundle) {
    ...
    const _arrangement = bundle && bundle.songInfo && bundle.songInfo.arrangement;
    if (_arrangement !== _cachedPaneKeyArrangement) {
      _cachedPaneKeyArrangement = _arrangement;
      _cachedPaneKey = _aspectPaneKey(_arrangement, _paneUid);
    }
    const _paneKey = _cachedPaneKey;
    ...
    let _camX = ctx.cam.curX + shoulderOffset; let _camY = h * _hMul; let
      _camZ = dist * _dMul;
    ...
    if (_freeCam && _freeCam.enabled) {
      ...
      const _tx = ctx.cam.curX; const _ty = ctx.cam.curLookY; const
        _tz = _lookAtZ; // look target
      let _vx = _camX - _tx; let _vy = _camY - _ty; let
        _vz = _camZ - _tz;
      _vx *= _distMul; _vy *= _distMul; _vz *= _distMul; // zoom (dolly)
      _vy *= _heightMul; // height
      const _cy = Math.cos(_yaw); const
        _sy = Math.sin(_yaw); // orbit around Y
      const _rx = _vx * _cy - _vz * _sy; const
        _rz = _vx * _sy + _vz * _cy;
    ...
    if (_freeCam && _freeCam.enabled) {
      ...
      if (_rowNdcY < FRET_ROW_FIT_NDC_MIN) {
        ...
        ctx.cam._fretRowFitBoost = Math.min(
