...
import {
  ...
  FRET_LABEL_GOLD_HEX, GHOST_FRET_LBL_FADE_S, GHOST_HOLD_AFTER_ONSET,
...
import {
  hwyPostHitTailFadeMul, renderOrderForLayerAtZ,
...
export function createNoteRenderer(deps) {
  ...
  function bendSemisAtTime(n, chartTime) {
    ...
    const RISE = BEND_ENV_RISE_FRAC; const
      REL = BEND_ENV_RELEASE_FRAC;
    let env;
  ...
  function drawNote(n, now, openX, skipLabel, skipBody, linger = 0.10, openChordBoxWidth, fromChord = false, chordId, susTrailMatchArpFrame = false, arpBounds = null, prevOnsetT = -Infinity, showDropLine = false, frame) {
    ...
    const {
      ...
      _showFingerHints, noteDetectGetState, noteDetectHasProvider,
    ...
    if (hasSus) {
      ...
      let _lvCur = noteDetectState;
      let _lvGood = noteDetectGood;
      if (!_lvCur) {
        ...
        if (!_lvCur) {
          for (let _mi = 0; _mi < noteDetectMissMarks.length; _mi++) {
            ...
            if (_mm.s === n.s && _mm.f === n.f && Math.abs(_mm.noteTime - n.t) < NOTEDETECT_TIME_EPS) {
              _lvCur = 'miss'; _lvGood = false; break;
            }
          }
        }
      }
      if (_lvCur) {
        ...
        _susVerdictLatch.set(_sk, _lvGood ? (_live ? 'hit-live' : 'hit') : 'miss');
    ...
    if (noteDetectGood) {
      ...
      if (fromChord) {
        ...
        if (!_fwE) {
          _fwE = {
            minF: Infinity, maxF: -Infinity, a: 0, openA: 0, t: n.t,
          };
    ...
    if (!effSkipBody && !arpGhostOnlyMode && !_overLinger) {
      ...
      if (noteDetectChordState) {
        ...
        if (noteDetectState === 'miss') {
          ...
          if (_verdictMarks) {
            noteDetectLabels.push({
              x, y: y + NH * 1.7, z: noteZ + 0.02, labels: [{ text: '✗', color: '#ff5a7a' }],
            });
          }
          ...
          if (_verdictMarks) {
            const _tc = _timingHex(noteDetectMatchedMark && noteDetectMatchedMark.timingState, frame); noteDetectLabels.push({
              x, y: y + NH * 1.7, z: noteZ + 0.02, labels: [{ text: '✓', color: `#${_tc.toString(16).padStart(6, '0')}` }],
            });
          }
      ...
      if (noteDetectGood && noteDetectChordStateIsObj
        ...
        _fxSpawnPop(
          noteDetectChordState.popKey,
          noteDetectChordState.points,
          noteDetectChordState.mult,
          x,
          y + NH * 2.2,
          noteZ + 0.02,
        );
    ...
    if (!effSkipBody && !arpGhostOnlyMode && !_overLinger) {
      ...
      if (n.f > 0 && !skipLabel) {
        ...
        if (!fromChord) {
          ...
          line.renderOrder = renderOrderForLayerAtZ(
            noteZ,
            _isArpNote
              ? 'ARP_CONNECTOR_LINE'
              : 'CONNECTOR_LINE',
        ...
        if (alpha > 0 && n.f > 0) {
          ...
          const _drawTeachMark = (text, colorHex, dx, cacheKey) => {
            ...
            spr.renderOrder = renderOrderForLayerAtZ(
              noteZ,
    ...
    if (n.f > 0 && projectionVisible && (
      ...
      const rimSolid = arpGhostActive
        ? 0.75 * arpGhostAlpha
        : projFactor * 0.94;
      proj.material.opacity = Math.min(
        0.96,
