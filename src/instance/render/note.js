...
// (ghost fret-digit template lookups, the slide ribbon, bend/vibrato Y
// offset). Moved verbatim out of createFactory() (Stage 7 Phase 3b) -- this
// was the single largest still-monolithic function (drawNote: 92 closure-var
// reads, 3 writes) and the plan's own pattern-validator for the "ctx"
// extraction style used from here on.
//
// Two dependency tiers, matching how each piece of state actually behaves:
//
// 1. `deps` (this factory's argument) -- injected ONCE, at construction time.
//    Everything here is either a) a Three.js material/geometry/pool that
//    initScene() builds fresh and never reassigns again until the next
//    re-init (verified: every one of these is only ever bare-reassigned
//    inside initScene()/teardown(), never per-frame), or b) a small shared
//    function (validString, _setLabelMap, ...) or Map/Set that's mutated via
//    its own methods (not reassigned) and lives on past this call. Call
//    createNoteRenderer() again each time initScene() reruns, right after
//    these are (re)built -- see main.js's call site, same pattern as
//    POOL_REGISTRY.
//
// 2. `frame` (drawNote's last parameter) -- values that genuinely change
//    frame-to-frame (camera position, active settings, notedetect snapshot
//    state) but are owned and written elsewhere (camUpdate/applySize,
//    loadSettings/the settings listener, update()'s per-frame notedetect
//    snapshot block). update() builds ONE frame object per call (not one per
//    note -- drawNote is called from two loops, once per visible note/chord
//    note) and passes the same reference to every drawNote() call that
//    frame, so there's no staleness risk and no per-note allocation. Do NOT
//    promote a `frame` field to a `deps` field just because it looks stable
//    -- `_fretLabelAllowed` looked construction-stable too until grepping
//    every reassignment site turned up a rebuild inside update(), before the
//    drawNote call sites, on chart change.
//
// The 3 true write-backs (noteDetectVerdictMaxAlpha, noteDetectVerdictSawAlpha,
// _streakHits) don't fit either tier -- they're written here AND read/reset
// by code elsewhere in update() on the same frame. Those live on
// `noteVerdictState`, a small object both sides hold the same reference to
// and mutate directly (no frame-bag sync-back needed).
export function createNoteRenderer(deps) {
    const {
        ...
        // Sprite/material caches (Phase 1b).
    ...
    function drawNote(n, now, openX, skipLabel, skipBody, linger = 0.10, openChordBoxWidth, fromChord = false, chordId, susTrailMatchArpFrame = false, arpBounds = null, prevOnsetT = -Infinity, showDropLine = false, frame) {
        // `frame` is a single object update() builds once per call (not once
        // per note) holding everything that varies frame-to-frame but isn't
        // owned by this module -- camera/settings/notedetect snapshot state.
        // See note-frame.js's doc comment for the full field list + why.
        const {
        ...
        if (!effSkipBody && !arpGhostOnlyMode && !_overLinger) {
            ...
            if (noteDetectChordState) {
                ...
                if (noteDetectState === 'miss') {
                    ...
                    noteVerdictState.streakHits = 0;            // #7 break the streak (heat eases down)
                    ...
                    if (_verdictMarks) { const _tc = _timingHex(noteDetectMatchedMark && noteDetectMatchedMark.timingState, frame); noteDetectLabels.push({ x, y: y + NH * 1.7, z: noteZ + 0.02, labels: [{ text: '✓', color: '#' + _tc.toString(16).padStart(6, '0') }] }); }  // #6 + #5
