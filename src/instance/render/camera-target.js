...
export function createCameraTarget({
  ...
  function drawCameraTarget(
    ...
    if (bootstrapHoldActive) {
      ...
      const lowBlendFs = 1 - (1 - CAM_FOCUS_BLEND_RATE) ** dBlend;

      if (!lookaheadHasBounds || lookaheadMaxF <= LOOKAHEAD_LOCK_ENGAGE_MAXF) ctx.cam._lookaheadHiNeckLatch = false;
      else if (lookaheadMaxF >= LOOKAHEAD_LOCK_RELEASE_MAXF) ctx.cam._lookaheadHiNeckLatch = true;
      ...
      if (lookaheadLockLowEligible) {
        ...
        if (lookaheadBoundsNow) {
          const tgtWX = lookaheadTargetWorldX(lookaheadBoundsNow.minF, lookaheadBoundsNow.maxF);
          const tgtSpanInt = Math.max(1, lookaheadBoundsNow.maxF - lookaheadBoundsNow.minF + 1);
