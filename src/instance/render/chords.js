 ...
 * for every visible chord. One continuous per-chord loop with ~30 locals
 * computed at the top of each iteration and consumed by every section
 * below it in the same iteration, plus loop-carried state comparing each
 * chord to the previous one in the same pass (`runSigPrev`/`prevChordSig`/
 * `prevChordTime`/`prevAnyChordTime` — despite the name, this is
 * intra-frame chord-to-chord comparison, re-initialised at the top of
 * `drawChords()` every call, not cross-frame persistence).
 *
 * Three dependency shapes:
 * 1. `deps` (injected once) — pools/materials/caches/functions that are
 *    construction-time stable, including 3 persistent Maps
 *    (`_chordVerdicts`, `_noteStreamBracketStrings`, `_ghostPrevBuf`)
 *    verified to only be reassigned wholesale in `initScene()`/`teardown()`
 *    or not at all.
 * 2. `frame` (parameter) — the same `_noteFrame` object `update()` already
 *    builds once per call for `noteRenderer.drawNote()`, extended with the
 *    extra fields this loop needs (`camT0`/`camT1`/`camTau`/`camHystF`/
 *    `camDistHystF`/`cameraMode`/`_leanSus`) — still only one frame bag per
 *    `update()` call.
 * 3. `accum` — a small mutable accumulator for the 6 primitives this loop
 *    and the single-notes loop both accumulate into across the same frame
 *    (`highwayIntensity`, `camWX`, `camWSum`, `camDistMin`, `camDistMax`,
 *    `camDistGot`), read afterward by the camera-target code. Neither
 *    `deps` (reset every frame) nor `frame` (genuinely written here) fits.
 *
 * `activeFrets` (Set) and `lastFretForString` (array) are plain per-call
 * parameters, mutated via `.add()`/index-assign only. `_scrAtMinFretArr`/
 * `_scrAtMinFretLen` (barre-detection scratch) and `_scrChordNote` (the
 * drawNote-call scratch object) are private state of this factory —
 * referenced nowhere else.
 */
export function createChordRenderer(deps) {
  const {
    pChordBox, pChordFrameFill, pChordLbl, pBarreLine, pNoteFretLabel,
  ...
  const _scrChordNote = {};
  /** Reusable scratch for barre atMinFretStrings computation — avoids a filter/map/sort allocation chain per chord per frame. */
  const _scrAtMinFretArr = new Array(MAX_RENDER_STRINGS).fill(0);
  let _scrAtMinFretLen = 0;
  /** Reusable scratch for the chord-base fret-number dedup pass — avoids a `new Set()` per chord per frame. */
  const _scrSeenChordFrets = new Set();
  ...
  function drawChords(chords, notes, anchors, bundle, now, t1, ndVerdictT0, activeFrets, lastFretForString, accum, frame) {
    ...
    if (chords) {
      ...
      // pre-loop scan. SHAPE_RUN_GAP_S resets the run when the time gap from the
      // previous chord exceeds the same 0.5s window isRepeat uses — a chord shape that
      // re-appears after a real musical gap should re-show its label, not continue a run.
      ...
      // Skip past chords too old to render. The per-chord filter (ch.t + _chFilterSus >=
      // ndVerdictT0) passes the earliest chord when ch.t >= ndVerdictT0 - AHEAD (worst
      // case: _chFilterSus = AHEAD for a chord with no explicit sustain). Binary search
      // avoids iterating hundreds of past chords every frame in dense PM/FH sections.
      const _chordsLoIdx = lowerBoundT(chords, ndVerdictT0 - AHEAD);
      // Prime shape-run tracking from the chord immediately before the window so
      // isRepeat and firstInShapeRun are correct on the first visible chord.
      if (_chordsLoIdx > 0) {
      ...
      for (let ci = _chordsLoIdx; ci < chords.length; ci++) {
        const ch = chords[ci];
        // Chords are time-sorted — everything beyond t1 is outside the visible window.
        if (ch.t > t1) break;
        ...
        if (runSig === null) {
          ...
          runSigPrev = runSig;
          // Only valid chords update the run-gap clock, or an entry with no
          // notes/usable chordId would make the next real chord look like a tiny gap.
          prevAnyChordTime = ch.t;
        }
        if (!ch.notes) continue;
        // Filter chord notes to in-range strings once — all chord-level aggregations
        // (maxSus, repeat signature, open-string centroid, frame-box bounds,
        // active-fret highlights, camera-window dist) read from chordNotes.
        ...
        if (ch.t > now && ch.t < now + 2) for (const cn of chordNotes) { if (cn.f > 0) activeFrets.add(cn.f); }

        let maxSus = 0;
        for (const n of chordNotes) if ((n.sus || 0) > maxSus) maxSus = n.sus;
        // When maxSus=0 (no explicit sustain, including all h3dSynth chords) use AHEAD
        // as the filter window so the chord stays in the loop long enough for a
        // handshape-derived sustain rail to finish drawing — the rail itself gates on
        // _dtSusEnd>0, so chords with no actual sustain produce no visual artifact.
        // ndVerdictT0 extends the window when a note-detect provider is attached so
        // async verdicts still land while drawable.
        const _chFilterSus = maxSus > 0 ? maxSus : AHEAD;
        if (ch.t + _chFilterSus < ndVerdictT0) continue;
        if (ch.t > t1) break;

        // Repeat-chord detection (consecutive same shape, short gap), reusing runSig
        // computed at loop entry. Synthetic chords (h3dSynth, injected at handshape
        // onsets) are never real strums, so they must not update
        // prevChordSig/prevChordTime — otherwise a real chord whose handshape
        // generates a synth onset at the handshape start_time could see the synth as
        // its "previous chord" and be falsely flagged isRepeat.
        ...
        // - Upcoming (chDtEarly > 0): onset time — frame previews the correct neck
        //   region before the chord hits the line.
        // - Past, actively sustaining (now < ch.t + maxSus): onset time — frame stays
        //   at the frets where the chord was struck; using `now` would jump the frame
        //   to whichever anchor is active at `now`, making the sustain box appear in
        //   the wrong fret zone.
        // - Past, linger-only (sustain expired or no sustain): `now` — the fade-out
        //   frame tracks the current lane position so it doesn't visibly drift.
        ...
        // Open-string X: chart <anchor> lane centre when present, matching highway span.
        let chordCX = ctx.cam.curX;
        if (chAncB) chordCX = (xFret(chAncB.dMin) + xFret(chAncB.dMax)) / 2;
        else {
          let cxL = Infinity; let cxR = -Infinity; let
            fretted = 0;
        ...
        // Horizontals for chord frame + open-string mesh width. With anchors, span
        // matches HWY lane columns (wire dMin..dMax); no extra pad.
        let chordFrameXL = null; let chordFrameXR = null; let
          chordOpenBoxW = null;
        let chordFrameAnchorMatched = false;
        if (chShape.size > 1) {
          let fMinCh = 99; let fMaxCh = 0; let
            anyFretted = false;
          ...
          // Prefer the anchor span so chord/arpeggio frames align with the highway
          // lane window — but only when the chord's fretted notes actually fall
          // within the anchor range, or the framebox would clip its own gems.
          ...
          if (chordFrameXL != null && chordFrameXR != null) {
            ...
            if (span > 1e-8) {
              // Anchor-driven lane stripes span [dMin..dMax] wire-to-wire with no
              // horizontal pad — match that only when actually following the
              // anchor (all-open fallback path). The fretted-span path always
              // pads so the frame breathes around the outermost fretted notes.
        ...
        const hsTimeWinFrame = hsHintFrame.hs
          ...
          : null;
        // chordShapeCoveredByStandaloneNotes is cached per chord (_arpCoverCache), so a
        // direct call from the deferChordGems short-circuit chain is lazy and O(1) when re-hit.
        const inferredArpPattern = (!hsHintFrame.hs
                    ...
                    && chordInference.inferArpeggioFromNotePattern(ch, chShape, notes, hsTimeWinFrame, bundle.handShapes);
        // Only suppress the chord gems when standalone notes really cover the
        // arpeggio shape; otherwise explicit/synth hand shapes can produce an empty
        // lavender frame with no notes inside. Lazy wrapper so the note-stream scan
        // is skipped when neither branch needs it.
        ...
        /**
                 * Lavender chord frame + purple highway rails: authored arpeggio metadata
                 * only. RS `highDensity` marks gallops/repeated strums on the same voicing,
                 * not arpeggio — kept for sustain-ribbon width via `chordSusTrailMatchArpFrame`.
                 *
                 * Only the chord that initiates the handshape span gets the lavender
                 * treatment — subsequent strums of the same voicing within the same
                 * handshape window render as ordinary chord frames. Proximity to
                 * `hsStart()` (<= 100ms) identifies the initiating chord regardless of span width.
                 */
        ...
        const chordHighwayLavenderArpVisual = hsHintFrame.explicit
                    && !Number.isNaN(_hsStartT) && Math.abs(ch.t - _hsStartT) <= 0.1;
        const chordSusTrailMatchArpFrame = chordInference.chordWireHighDensity(ch)
                    || chordHighwayLavenderArpVisual;

        // Onset in window OR chord started before the window but is still sustaining
        // right now. Gated against the current frame time so camera framing releases
        // as soon as the chord is no longer rendered on screen.
        ...
        // Symmetric decay — see the matching comment in the single-note branch. The
        // chord-wide chW uses ch.t (not per-note onset) since chord notes share a strum time.
        const chW = chWindowed ? Math.exp(-Math.abs(ch.t - now) / camTau) : 0;
        // Next-chord tail: same voicing (highDensity gallop) keeps full linger +
        // optional fade suppression inside [hold-fade, hold]; a voicing change clips
        // the tail to the chart gap so two adjacent frames don't stack.
        let cjNext = null;
        ...
        // Nearest following event (chord OR single note) — used by chordTailMul so the
        // framebox vanishes the moment any next event is played. Pulled from the same
        // sorted scalar scratch drawNote uses (O(log N) over the shared 2*nStr buffer).
        ...
        const chordNextSoon = false;
        if (cjNext && cjNext.t > ch.t + 1e-6) {
          // Clip the hold tail to the gap for both same-voicing (repeat) and
          // different-voicing chords. chordTailMul's instant-cut check handles the
          // precise zero at onset; the clipped holdS prevents the outer gate and
          // hwyPostHitTailFadeMul from lingering past that point.
          chordTailHoldS = Math.min(CHORD_HWY_LINGER_S, Math.max(cjNext.t - ch.t, 1e-3));
        }
        // Engine verdicts land ~0.4s after the chord crosses (feedBack#254); on a fast
        // different-voicing sequence the clip above can shrink the rim's draw life
        // below that, so the latch is set but the rim isn't drawn anymore. When a
        // verdict provider is attached, floor the hold at NOTEDETECT_GEM_VERDICT_WINDOW
        // so the tinted rim is actually visible. This deliberately overrides the
        // voicing-change clip: the post-hit z clamp pins extended frames at z=0, so two
        // frames can overlap in plane and are distinguished by their tinted rim color
        // instead — in detect mode, verdict visibility beats the cleaner approach silhouette.
        ...
        // When an authored arpeggio chord frame is still approaching, only the first
        // note to be played is shown as a gem — all others are suppressed until
        // chDtEarly <= 0 so the frame doesn't flood the view with simultaneous gems.
        // The first note is the earliest match in the note stream within the handshape
        // window. If no note-stream note matches the chord shape (no sequential
        // arpeggio pattern), _arpApproachFirstNote stays null and all chord gems show
        // — handles chords played simultaneously even when tagged as arp.
        ...
        if (chordHighwayLavenderArpVisual && !deferChordGems
          ...
          // notes matching this shape, the chord is played simultaneously.
        }

        // ── Deferred-arpeggio gem fallback ─────────────────────────────────
        // When gems are deferred to the note stream (deferChordGems=true) but no
        // individual note matching the chord shape falls within the chord's onset
        // cluster window, the frame box has no gems at its Z position — show all
        // chord gems as a preview so the frame box isn't empty. Uses the same onset
        // window as chordShapeCoveredByStandaloneNotes so the fallback deactivates
        // precisely when the stream truly covers the onset. Inlined to skip a
        // per-chord closure allocation.
        ...
        // Suppress gems AND frame for hand-shape-synthesized chords whose notes are
        // already rendered individually via the note stream — showing chord gems or a
        // framebox for a synth chord that duplicates the note stream looks like
        // phantom notes/chords. Check: any standalone note matching any shape string
        // in the onset window. Weaker than chordShapeCoveredByStandaloneNotes (all
        // strings covered) to handle patterns where one shape string only appears well
        // after the onset cluster. Inlined for the same reason as _deferFallback above.
        ...
        // skipBody=true so the board projection (fret ghost) renders for all shape
        // strings — shows the hand position like a chord would.
        // chordLinksSlide: true when any chord note has a direct sl/slu marker, or when
        // the chord's sustain connects (via case-2 linkNext) to a note in bundle.notes
        // that has a slide. Repeated chords matching either condition render as normal
        // chords so the player sees the gem.
        let chordLinksSlide = chordNotes.some((cn) => (Number.isFinite(cn.sl) && cn.sl >= 0)
        ...
        if (!deferChordGems || _deferFallback || suppressSynthChord) {
          for (const cn of chordNotes) {
            // Suppress non-first gems while an authored arpeggio frame approaches
            // — but not for the deferred fallback path, where all chord gems serve
            // as the only visual preview. _arpApproachFirstNote is null when no
            // sequential pattern was found, so simultaneous chords are unaffected.
            ...
            // Only suppress labels on repeated chord shapes (not first-in-run).
            const skipLabel = !firstInShapeRun;
            // Reuse _scrChordNote scratch instead of `{ ...cn }` spread (avoids
            // per-chord-note allocation every frame).
            Object.assign(_scrChordNote, cn);
            _scrChordNote.t = ch.t;
            _scrChordNote.sus = cn.sus || 0;
            // `fhm` is omit-when-false on the wire (unlike mt/pm, always emitted),
            // so Object.assign leaves a stale `true` from a previous muted chord
            // note untouched — reset it explicitly.
            _scrChordNote.fhm = cn.fhm || false;
            // Same stale-scratch hazard for the bend shape: bnv/bt are
            // omit-when-default on the wire, so a chord note without them would
            // otherwise inherit the previous note's curve. Reset explicitly.
            _scrChordNote.bnv = Array.isArray(cn.bnv) ? cn.bnv : undefined;
            _scrChordNote.bt = cn.bt || 0;
            // Same stale-scratch hazard for the teaching marks: fg/sd are
            // omit-when-default, so a chord note without them must reset to -1.
            ...
            // Gate by this note's own sustain against the current render time —
            // drawNote has already dropped short-sustain notes whose ringing has
            // ended, so they shouldn't keep pulling the camera frame wider than
            // the notes actually still on screen (chord-wide maxSus would
        ...
        // (chordHighwayLavenderArpVisual). Covers gems shown directly from the chord
        // (!deferChordGems) and the deferred-fallback preview path (_deferFallback).
        // The inferred-arpeggio path (inferredArpPattern only, no explicit mark)
        // intentionally does NOT draw brackets — the inference heuristic can
        // false-positive on fast strummed chords, and brackets on non-arp chords
        // confuse players. Note-stream arpeggios draw their own brackets in the
        // notes[] loop above; this chord loop covers strings whose notes haven't
        // entered AHEAD yet, and _nsBrackets prevents duplicates.
        if (chordHighwayLavenderArpVisual) {
          const _arpBracketDt = ch.t - now;
          if (_arpBracketDt < AHEAD) {
            const _arpEnd = (hsHintFrame.hs && !Number.isNaN(chordInference.hsEnd(hsHintFrame.hs)))
              ? chordInference.hsEnd(hsHintFrame.hs)
              : ch.t + maxSus + CHORD_HWY_LINGER_S;
            // The notes[] loop already drew brackets for any note-stream note that
            // entered AHEAD, recording (chordId:occurrenceStart -> strings) in
            // _noteStreamBracketStrings. Same composite key (template id + handshape
            // start time) so two arp occurrences sharing a chord template ID are
            // distinct occurrences, not mutually suppressing.
            const _nsBracketsKey = `${ch.id}:${_hsStartT}`;
            const _nsBrackets = _noteStreamBracketStrings.get(_nsBracketsKey);
            // Open-string bracket X: anchor at handshape start so position stays
            // fixed even when chordCX drifts with now.
            const _arpChBrktAncB = !Number.isNaN(_hsStartT)
            ...
            const _arpChBrktOpenW = (() => {
              if (_arpChBrktAncB) {
                const _xl = xFret(_arpChBrktAncB.dMin); const
                  _xr = xFret(_arpChBrktAncB.dMax);
        ...
        const chordTailMul = (() => {
          // When a next event (chord OR single note) has already crossed the hit
          // line, hide this frame immediately — no fadeout overlap with a playing event.
        ...
        if (chShape.size > 1 && chDt > -chordTailHoldS && chDt < AHEAD && chordOpenBoxW != null
          ...
          // Repeat frames use half height but anchor at yMinF (board level) rather
          // than centering in the string range — with the camera tilted downward, a
          // centered half-height frame puts its bottom bar mid-strings (far above
          // the board), causing perspective-induced apparent X-misalignment with the
          // lane tiles. Anchoring at yMinF keeps both at the same projected screen X.
          const yBot = yMinF;
          ...
          // not bar thickness vs first chord.
          let ft = Math.max(CHORD_FRAME_RIM_MIN * K, fullChordBoxH * CHORD_FRAME_RIM_FRAC_H);
          if (chordAccent) ft *= 1.22;
          // Lavender frame: authored arpeggio marker only — RS highDensity is kept
          // out, since it tags gallops/repeated strums, not arpeggio.
          ...
          // Capture the neutral frame color before any verdict overwrite — used for
          // the mute X lines so hit/miss feedback only shows on the outer borders.
          const baseRimHex = rimHex;
          // Once the chord crosses the hit line, tint the teal frame by the
          // note-state provider verdict (feedBack#254): green on a clean grab, red on
          // a miss. The verdict is async (~0.4s after the line), so the frame stays
          // teal while pending — it must not flash red before the verdict lands. The
          // green/red verdict is latched in _chordVerdicts so it can't flicker as
          // constituent glows decay. Only engages when a scorer is attached; arpeggio
          // frames keep their blue identity. verdictKey composes ch.id (the template
          // id, reused across same-shape occurrences) with ch.t for one entry per
          // physical onset.
          const verdictKey = _encodeChordVerdictKey(ch);
          // Evict any stale latch the next time the chord re-enters the pre-hit
          // window (rewinds, section loops, full restarts). Bounds Map growth too.
          if (chDt > 0 && _chordVerdicts.has(verdictKey)) {
            _chordVerdicts.delete(verdictKey);
          }
          // The verdict scan doesn't skip authored-handshape frames — power chords
          // sometimes carry an explicit handshape (RS authoring quirk), which would
          // otherwise drop them into the isArpeggioFrame path and leave them
          // lavender-blue regardless of hit/miss. A true arpeggio (handshape over a
          // real sweeping note run) is unaffected: its constituents are standalone
          // notes judged at their own times, so the scan at ch.t finds nothing and
          ...
          if (chDt <= 0 && noteDetectHasProvider && !isArpeggioFrame) {
            ...
            if (latched === 'green') {
              ...
              // The first scan past the verdict window came up empty (no
              // constituent ever had a state — most often a true arpeggio frame
              // whose notes are judged at their own times, not at ch.t). Skip the
              // per-frame provider scan and keep the frame's default identity.
            } else {
              // Latch both green AND red:
              //   - any constituent 'miss' -> red latched. One decisive miss means
              //     the chord can't be all-hit; without latching, the rim would
              //     fall back to teal once noteStateFor's miss-wash window (~0.6s
              //     TTL) expires and the state returns null again.
              //   - all hit/active -> green latched.
              //   - else (no miss yet, some constituents still null) -> keep teal
              //     default. A partial state must not flash red on a chord whose
              //     verdicts arrive incrementally.
              ...
              for (const cn of chordNotes) {
                ...
                if (st === 'hit' || st === 'active') {
                  ...
                  // First miss decides the chord — short-circuit further
                  // provider calls in chord-dense passages.
                  allHit = false;
                  anyMiss = true;
                  anyState = true;
                  break;
              ...
              if (anyMiss) {
                ...
                // The engine verdict typically lands ~0.4s after the chord
                // crosses the line; by NOTEDETECT_UNMATCHED_LATCH_AFTER we've
                // waited well past that window. If no constituent ever returned
                // a non-null state by then, there's no verdict coming for this
                // chord (true arpeggio frames: notes are judged at their own
                // times, never at ch.t). Latch 'unmatched' so subsequent frames
                // skip the provider scan entirely. The threshold must fall
                // inside the chord frame's visible draw window —
                // chordTailHoldS is floored to NOTEDETECT_GEM_VERDICT_WINDOW
                // (0.75s) in detect mode, so frames stop drawing at chDt <
                // -0.75; a threshold at exactly -0.75 would be unreachable
                // because the draw gate kicks the frame out first. Placed
                // ~0.55s past the line so it fires for ~0.2s of the remaining
                // visible window. The !anyState guard keeps a partial-resolve
                // case (one cn 'hit', another still null) scanning until
          ...
          if (chDt > 0) { // framebox only on highway, not on the fretboard
            ...
            // Per-depth layer stack: chord frames, gems, technique markers, and fret
            // labels all derive from RENDER_ORDER_LAYER_STACK. Sub-increments of
            // 0.0001 for intra-chord ordering, safe for any chord gap >= 0.001s.
            ...
            // Swapping `map` between two non-null gradient textures doesn't change
            // shader-defining state, so no needsUpdate (that would force a recompile per frame).
            ...
            // Non-repeat tapers the upper side bars + draws a thin top bar; hoisted so
            // ySideHi matches the top-bar thickness (using ft would leave a visible gap
            // between the thin top bar and the side bars meeting it).
            ...
            // Accent bloom on frame edges: 4 additive shells with Gaussian-style
            // falloff. Each border expands only in its perpendicular axis so bloom
            // never leaves the frame boundary: horizontal bars expand Y only,
            // vertical bars expand X only.
            if (chordAccent && pHaloBar) {
              // Bloom only on the teal (thin) parts of the frame — the dark L-corners
              // are deliberately left without bloom so they stay visibly dark.
              ...
              if (centerBotW > 0) drawHaloBar(cx, yBot + ft * 0.5, centerBotW * 0.5, ft, 0);
              // Top bar
              if (withTopFrame) drawHaloBar(cx, yTop - ftThin * 0.5, width * 0.5, ftThin, 0);
            ...
            // Harmony annotations: the chord's function (fn.rn Roman numeral) and
            // template voicing, stacked above the chord name. Gated by the
            // teaching-marks opt-in (mirrors the 2D overlay). Display only — never grading.
            if (_drawTeachingMarks && firstInShapeRun && !chordInference.chordWireHighDensity(ch)) {
              ...
              if (_h.rn || _h.voicing || _h.caged || _h.guideTones) {
                ...
                // Start one chord-name-height above the name and stack upward so
                // labels never overlap the gems.
            ...
            // Shape-based barre detection for the 3D indicator, driven off chord notes
            // alone (independent of label availability). Matches drawChordDiagram
            // PATH A + PATH B so the highway line and overlay bracket always agree:
            //   PATH A: 2+ adjacent strings at the minimum fret.
            //   PATH B: outer-edge full-span barre where the two outer strings are at
            //           the minimum fret, every intermediate string is fretted (f>0),
            //           and no intermediate string also sits at the minimum fret.
            {
              let bFret = Infinity;
              for (const [, f] of chShape) {
              ...
              if (_scrAtMinFretLen >= 2) {
                ...
                if (maxS - minS >= MIN_BARRE_SPAN_3D) {
                  // chShape is already a Map<s, f> — query it directly instead of
                  // building a transient Set<s> every frame.
                  let allFretted = true;
                  ...
                  if (allFretted) {
                    if (is3dBarre) {
                      ...
                      // PATH A did not fire: only draw if no inner string
                      // also sits at the minimum fret.
            ...
            // Show fret number per unique fretted position for non-repeated chords so
            // the player can read the shape at a glance.
            if (!isRepeat) {
              const _chFretLblAlpha = Math.min(1.0, (AHEAD - chDt) / 0.35) * chordTailMul;
              _scrSeenChordFrets.clear();
              for (const [, f] of chShape) {
                if (f <= 0 || _scrSeenChordFrets.has(f)) continue;
                _scrSeenChordFrets.add(f);
            ...
            // Per-chord Z-proportional renderOrder: muted fill/lines and frame edges
            // all use the named layer offsets above.
        ...
        // Left + right rail as plane meshes in the WebGL scene so they respect
        // renderOrder (16) and never occlude note gems (20/21). isRepeat chords also
        // draw their rail: each repeat shows a segment from its own onset to the next
        // chord's onset (or the handshape end, whichever is shorter), chaining
        // together to cover the full handshape duration visually.
        if (chShape.size > 1 && chordOpenBoxW != null && chDt < AHEAD) {
          // Cap handshape-derived sustain at the gap to the next chord — each chord
          // (including repeats) only extends to the next chord's onset, so the rail
          // never lingers past the anchor region of the current chord.
          const _nextChordGap = (ci + 1 < chords.length)
            ? chords[ci + 1].t - ch.t
            : Infinity;
          // Use the time remaining in the handshape from THIS chord's onset (hsEnd -
          // ch.t), not the full handshape span — when multiple chords share the same
          // handshape window, each chord after the first starts mid-handshape, and
          // using the full span would give a rail extending far past the handshape
          // end, visually swallowing subsequent single notes.
          ...
          // Apply the 0.4s visual-minimum only to chords with an explicit note
          // sustain (maxSus > 0). Handshape-derived sustain (_hsSus, already capped
          // at _nextChordGap) must not be inflated — that would undo the gallop cap.
          const _effSus = maxSus > 0
            ? Math.max(_rawSus, 0.4)
            : _rawSus;
          const _dtSusEnd = chDt + _effSus;
          if (_dtSusEnd > 0) {
            // Clip the rail at the next anchor boundary so it doesn't extend into a
            // different fret zone — the lane (pLane) slices correctly per-anchor, but
            // a single-segment rail at fixed X would visually invade the neighbouring
            // region when anchors change within the sustain window.
            ...
            if (anchors && anchors.length) {
              ...
              if (getChartAnchorAt(anchors, _susAbsT)
                ...
                if (_lo < anchors.length) _dtSusEndRail = anchors[_lo].time - now;
              }
            }
