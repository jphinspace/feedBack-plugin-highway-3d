...
export function createArpeggioLaneRail({ chordInference, validString, filterValidNotes }) {
  ...
  function fillArpeggioRailShapeBoundsCaches(handShapes, chords, chordTemplates, notesArr, laneRailFlags, loOut, hiOut) {
    const nHs = handShapes.length;
    for (let i = 0; i < nHs; i++) {
      if (!laneRailFlags[i]) continue;
      const b = effectiveArpRailChartBoundsForHandShape(handShapes[i], chords, chordTemplates, notesArr);
      loOut[i] = b.shapeLo;
      hiOut[i] = b.shapeHi;
    }
  }
  ...
  function arpeggioLaneOuterRailLaneSlice(
    ...
    return arpeggioLaneOuterRailChartIntervalOverlaps(tLo, tHi, handShapes, boundLo, boundHi, laneRailFlags);
  }
  ...
  /**
     ...
     */
  function arpeggioLaneOuterRailAtChartTime(chartT, handShapes, boundLo, boundHi, laneRailFlags) {
    return arpeggioLaneOuterRailChartIntervalOverlaps(chartT, chartT, handShapes, boundLo, boundHi, laneRailFlags);
  }
