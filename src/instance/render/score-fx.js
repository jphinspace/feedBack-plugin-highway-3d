...
export function createScoreFx({
  ctx, getCam, getProbe, sY, getNStr, noteDetectLabels, getNoteDetectFrameNowMs,
}) {
  ...
  const _fxBursts = Array.from({ length: 4 }, () => ({
    active: false,
    bornMs: 0,
    px: new Float32Array(_FX_BURST_N),
    py: new Float32Array(_FX_BURST_N),
    vx: new Float32Array(_FX_BURST_N),
    vy: new Float32Array(_FX_BURST_N),
  }));
  ...
  function drawScoreFx(ctx2d, W, H) {
    const cam = getCam(); const _probe = getProbe(); const
      nStr = getNStr();
