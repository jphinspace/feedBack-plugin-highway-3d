...
export function createTechMaterialCache() {
  ...
  function triMat(up, hex) {
    ...
    g.beginPath();
    if (up) { g.moveTo(S / 2, m); g.lineTo(S - m, S - m); g.lineTo(m, S - m); } else { g.moveTo(S / 2, S - m); g.lineTo(S - m, m); g.lineTo(m, m); }
    g.closePath();
    g.lineJoin = 'round';
    g.fillStyle = '#ffffff';
    g.fill();
  ...
  function slideArrowMat(pointRight, hex) {
    ...
    const g = c.getContext('2d');
    g.beginPath();
    if (pointRight) { g.moveTo(S - m, S / 2); g.lineTo(m, m); g.lineTo(m, S - m); } else { g.moveTo(m, S / 2); g.lineTo(S - m, m); g.lineTo(S - m, S - m); }
    g.closePath();
  ...
  return {
    triMat, bendChevronMat, slideArrowMat, disposeAll,
  };
}
