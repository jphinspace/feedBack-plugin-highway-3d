...
export function createChordAccentVisuals({
  ...
  let gPMXFill; let imPMXFill; let
    _imPMXFillMat;
  {
    // prettier-ignore
  ...
  // PM fill — InstancedMesh (black, varying alpha per chord).
  {
    ...
    _imPMXFillMat = new T.ShaderMaterial({
      vertexShader: _imFillVert,
      fragmentShader: _imFillFrag,
      transparent: true,
      depthTest: false,
      depthWrite: false,
      fog: false,
  ...
  let gFHXFill; let imFHXFill; let
    _imFHXFillMat;
  {
    // prettier-ignore
    const pos = new Float32Array([
  ...
  if (!gPMXLines) {
    ...
    for (let i = 0; i < XLINES.length; i++) {
      ...
      const dx = xb - xa; const
        dy = yb - ya;
      const il = 1 / Math.sqrt(dx * dx + dy * dy);
      const nx = -dy * il; const
        ny = dx * il;
      const vi = i * 12; const ii = i * 6; const
        vb = i * 4;
    ...
    _imPMXLinesMat;
  {
    ...
    _imPMXLinesMat = new T.ShaderMaterial({
      vertexShader: _imLinesVert,
      fragmentShader: _imLinesFrag,
      transparent: true,
      depthTest: false,
      depthWrite: false,
      fog: false,
  ...
  const mBarre = new T.MeshLambertMaterial({
    color: 0xffffff, emissive: 0xffffff, emissiveIntensity: 0.9 * glowMul, transparent: true, depthWrite: false,
  });
  ...
  const pConnectorLine = pool(noteG, () => new T.Line(
    ...
    new T.LineBasicMaterial({
      color: 0xaaaaaa, transparent: true, opacity: 0.5, depthTest: false,
    }),
  ));
