...
import {
  _recomputeFretSpacingDerived, fretLabelScaleForFret, initFretSpacing, setFretUniform,
} from './core/fret-geometry.js';
import {
  _noteKey, _openStringPitchLabelsForTuning, anchorLaneBoundsAt, getChartAnchorAt, lowerBoundT, resolveStringCount,
...
import {
  _aspectPaneKey, _aspectRegisterPane, _resolveTuneFor, nextPaneCounter,
...
import {
  ZERO_AUDIO_BANDS, getAudioAnalyser, readAudioBands, _resetAnalyserBridgeForTest,
} from './audio/analyser.js';
import { BACKGROUND_STYLES } from './background/styles/index.js';
import { createHudsCache } from './instance/overlay/huds.js';
import { createLyricsCache, drawLyrics } from './instance/overlay/lyrics.js';
import { createChordDiagramCache } from './instance/overlay/chord-diagram.js';
...
import { createChordInference } from './instance/model/chord-inference.js';
...
import { createFretColumnMarkerPool, createHighwayLanePlane, createLaneDividers } from './instance/geometry/lane-and-labels.js';
...
function createFactory() {
  ...
  let scene = null; let cam = null; let
    ren = null;
  let wrap = null;
  ...
  let _ctxLost = false;
  let _onCtxLost = null; let
    _onCtxRestored = null;
  let bcCtrl = null; // Butterchurn audio-reactive background (the 'butterchurn' bg-style)
  let _chartEnv = 0; let _chartPrevT = -1; let butterchurnBeatIdx = 0; let butterchurnNoteIdx = 0; let butterchurnChordIdx = 0; let
    butterchurnTintTarget = null;
  let _tintR = 20; let _tintG = 24; let
    _tintB = 40; // smoothed instrument-color tint for the bg
  ...
  let ambLight = null; let
    dirLight = null;
  let fretG = null; let tuningLblG = null; let noteG = null; let beatG = null; let
    lblG = null;
  let gNote = null; let gSus = null; let gBeat = null; let
    gTapChevron = null;
  ...
  let gNoteGrad = [];
  let mStr = []; let mGlow = []; let mSus = []; let mStrHitOutline = []; let mAccentOutline = []; let mAccentCore = []; let mAccentHaloNear = []; let mAccentHaloMid = []; let
    mAccentHaloFar = [];
  ...
  /**
     ...
     */
  const noteVerdictState = { maxAlpha: 0, sawAlpha: false, streakHits: 0 };
  ...
  function _v3TopRightChromeBottom() {
    ...
    const { top } = highwayCanvas.getBoundingClientRect();
    let maxBottom = 0;
  ...
  /** Section/tone HUD text-metrics cache — see instance/overlay/huds.js. */
  const hudsCache = createHudsCache();
  let pSusRail = null; let gSusRail = null; let
    mSusRailBase = null;
  let pSusRailBloom = null; let gSusRailBloom = null; let mSusRailBloomBase = null; let
    _bloomGaussTex = null;
  let pTechPlane = null; let
    gTechPlane = null;
  ...
  let imPMTech = null; let
    imFHTech = null;
  let _imGPMTech = null; let
    _imGFHTech = null; // cloned geometries (own instanceAlpha attr)
  let _imPMTechMat = null; let
    _imFHTechMat = null;
  ...
  let imPMXFill = null; let imPMXLines = null; let imFHXFill = null; let
    imFHXLines = null;
  let _imPMXFillMat = null; let
    _imPMXLinesMat = null;
  let _imFHXFillMat = null; let
    _imFHXLinesMat = null;
  ...
  let _imPMXFillCount = 0; let _imPMXLinesCount = 0; let _imFHXFillCount = 0; let
    _imFHXLinesCount = 0;
  ...
  let _imM4 = null; let _imPos = null; let _imSca = null; let _imQ = null; let _imAZ = null; let
    _imColor = null;
  ...
  const _fretMarkerWaveCache = new Map();
  ...
  let _sparkPts = null; let _sparkBurst = null; let
    _sparkUpdate = null;
  ...
  let noteDetectOnHit = null; let
    noteDetectOnMiss = null;
  let noteDetectOnBusHit = null; let
    noteDetectOnBusMiss = null;
  ...
  // Object pools
  let pNote; let pSus; let pLbl; let pBeat; let
    pSec;
  let pFretLbl; let pLane; let
    pLaneDivider;
  /** Shared lane-stripe materials, built in initScene(); hoisted so draw() can assign them per-stripe. */
  let mLaneOdd = null; let
    mLaneEven = null;
  /** Lane fret dividers: default white vs arpeggio frame tint on outer wires only. */
  let mLaneDivider = null; let mLaneDividerArp = null; let
    mLaneDividerExt = null;
  let pGhostFretLbl = null;
  ...
  const _laneSegArp = [];
  const _laneSegLen = 0;
  let pChordBox; let pChordFrameFill; let pChordLbl; let pBarreLine; let pArpBracket; let pPMXFill; let
    pFHXFill;
  ...
  let gPMXLines = null; let
    pMuteXLines = null; // PM X lines combined geometry (8 segs as quads)
  let gFHXLines = null; let
    pFHXLines = null; // FH X lines combined geometry
  let pNoteFretLabel; let pConnectorLine; let pDropLine; let pTapChevron; let
    pAccentHalo;
  let pTeachMarkLbl; // teaching marks fg/sd label sprites (§6.2.2)
  let pHaloBar = null; let
    gHaloBar = null; // gradient halo bar geometry — replaces per-shell pChordAccentHalo
  let gArpBracket = null; // shared 1×1×1 box geometry for pArpBracket; built once, disposed in teardown
  let pSusRibbon = null; let
    pSusRibbonOl = null;
  let pFretColMarker;
  ...
  let _lastSyncTuningRef;
  let _lastSyncBundleTuningRef;
  ...
  const fretLastActiveTime = new Array(NFRETS + 1).fill(0);
  ...
  /**
     ...
     * between distinct audio samples, and falls back to the raw value on pause/seek/stall so
     * the scroll never drifts against silent audio.
     *
     * The rate is only ever observable indirectly (one quantised bundle.currentTime sample
     * gap per ~16-23ms), which is noisy enough on its own to alternate between roughly 0.7x
     * and 1.4x every sample even during steady 1x playback — so _clkRate is EMA-smoothed
     * (CLK_RATE_EMA_ALPHA) rather than overwritten outright, and _frameNow is phase-locked
     * (CLK_OUTPUT_CORRECTION_PER_SEC) rather than snapped straight to each new raw sample:
     * it keeps extrapolating from its own last value and is pulled toward the newly-computed
     * target with a bounded per-second correction. Snapping to a fresh anchor whose rate
     * estimate has any error can jump backward relative to what the previous frame already
     * extrapolated to — that's the "shimmer" this pair of fixes exists to prevent. A genuine
     * discontinuity (seek, loop, pause, song change, stall) must still resync immediately, not
     * glide — every branch that resets _clkAudioT/_clkPerf/_clkRate outside of steady playback
     * also resets _clkOutputT to NaN so the next call reseeds instead of chasing a stale value.
     */
  const CLK_RATE_EMA_ALPHA = 0.1;
  const CLK_OUTPUT_CORRECTION_PER_SEC = 2.0;
  let _clkAudioT = NaN; // last distinct bundle.currentTime sample
  let _clkPerf = NaN; // performance.now() when that sample arrived
  let _clkRate = 1; // EMA-smoothed chart-seconds per real-second
  let _clkOutputT = NaN; // last value smoothNow() returned (NaN = reseed from the raw target)
  let _clkOutputPerf = NaN; // performance.now() at which _clkOutputT was computed
  ...
  function _openStringLabelSignature(bundle, labels) {
    ...
    const capo = bundle && Number.isFinite(bundle.capo) ? bundle.capo
  ...
  function _syncOpenStringPitchLabels(bundle) {
    ...
    const capo = Number.isFinite(bundle.capo) ? bundle.capo
      : (si && Number.isFinite(si.capo) ? si.capo : 0);
  ...
  /**
     ...
     */
  const _perfBench = (() => {
    try { return new URLSearchParams(window.location.search).get('h3dbench') === '1'; } catch (_) { return false; }
  })();
  let pbBeg; let pbEnd; let
    pbReportTick;
  if (_perfBench) {
    ...
    let _pbFrameCount = 0;
    pbBeg = function pbBegImpl(idx) { _pbStart[idx] = performance.now(); };
    pbEnd = function pbEndImpl(idx) {
      _pbAcc[idx].push(performance.now() - _pbStart[idx]);
    };
    pbReportTick = function pbReportTickImpl() {
    ...
    pbBeg = pbEnd = pbReportTick = function noop() {};
  }
  ...
  function initScene() {
    ...
    scoreFx = createScoreFx({
      ...
      getNStr: () => nStr,
      noteDetectLabels,
      getNoteDetectFrameNowMs: () => noteDetectFrameNowMs,
    });
    ...
    fretMarkersBuilder = createFretMarkersBuilder({
      ctx, xFret, xFretMid, textSprites,
    });
    ...
    materialRetint = createMaterialRetint({
      ...
      getMAccentHaloMid: () => mAccentHaloMid,
      getMAccentHaloFar: () => mAccentHaloFar,
    ...
    ({
      ...
      activePalette: ctx.settings.activePalette,
      glowMul: ctx.settings.glowMul,
      noteG,
      _recolorGemGradients: materialRetint.recolorGemGradients,
      _ownedSharedGeos,
      gHaloBar,
    }));
    _fwHitGlow.fill(0);
    ...
    ({
      ...
      noteG, gNote, mStr, mEdgeTransparent, mAccentHaloFar, gSus, mSus, mSusOutline,
    }));
    ({
      mTapChevron, pTapChevron, pLbl, pBeat, pSec,
    } = createTapChevronAndLabelPools({
    ...
    ({
      pFretLbl, mLaneOdd, mLaneEven, pLane, pGhostFretLbl,
    } = createHighwayLanePlane({
      noteG, lblG, textSprites, _ownedSharedMats, _ownedSharedGeos,
    }));
    ...
    noteRenderer = createNoteRenderer({
      ...
      techMaterials,
      validString,
      _setLabelMap: setLabelMap,
      _firstEventTimeGreaterThan,
      _fxSpawnPop: scoreFx.fxSpawnPop,
      _sparkBurst,
      xFretMid,
      sY,
      noteVerdictState,
    ...
    noteCameraTargets = createNoteCameraTargets({
    ...
    fretNumberRow = createFretNumberRow({
      pFretLbl, textSprites, sY, xFretMid,
    });
    ...
    settingsListener = createSettingsListener({
      ...
      _disposeOpenStringPitchSprites,
      _applyVibrancy: materialRetint.applyVibrancy,
      _applyGlow: materialRetint.applyGlow,
    ...
    ({
      ...
      NOTEDETECT_TIME_EPS,
      NOTEDETECT_TTL_MS,
      _fxHandle: scoreFx.fxHandle,
      _fxResolvePalette: scoreFx.fxResolvePalette,
  ...
  function loadSettings() {
    ...
    for (const key in LOAD_SETTINGS_SIMPLE_KEY_TO_FIELD) {
      if (!Object.prototype.hasOwnProperty.call(LOAD_SETTINGS_SIMPLE_KEY_TO_FIELD, key)) continue;
  ...
  function buildBoard() {
    ...
    for (let s = 0; s < nStr; s++) {
      ...
      const mat = new T.MeshStandardMaterial({
        color: ctx.settings.activePalette[s],
        emissive: ctx.settings.activePalette[s],
        emissiveIntensity: 0.002,
        transparent: true,
        opacity: ctx.settings._vibrancyIdleOp,
        roughness: 1,
      });
  ...
  function smoothNow(bundle) {
    ...
    if (raw !== _clkAudioT) {
      if (!Number.isNaN(_clkPerf)) {
        ...
        if (dP > 0.001 && dP < 0.5) {
          ...
          if (r > 0.05 && r < 5) {
            _clkRate += CLK_RATE_EMA_ALPHA * (r - _clkRate);
          } else {
            _clkRate = 1; // seek/loop — resync immediately, don't glide
            _clkOutputT = NaN;
          }
        } else if (dP >= 0.5) {
          _clkRate = 1; // long gap (paused / tab inactive) — resync immediately
          _clkOutputT = NaN;
        }
      }
      _clkAudioT = raw;
    ...
    // Target: where the (smoothed) rate says the clock should be right now. Phase-locked
    // below rather than returned directly — see the _clk* state block's doc comment.
    const target = _clkAudioT + _clkRate * dt;
    if (Number.isNaN(_clkOutputT)) {
      _clkOutputT = target;
    } else {
      const outDt = (p - _clkOutputPerf) / 1000;
      const extrapolated = _clkOutputT + _clkRate * outDt;
      const gap = target - extrapolated;
      const pull = Math.min(Math.abs(gap) * CLK_OUTPUT_CORRECTION_PER_SEC * outDt, Math.abs(gap));
      _clkOutputT = extrapolated + Math.sign(gap) * pull;
    }
    _clkOutputPerf = p;
    return (_frameNow = _clkOutputT);
  }
  ...
  function update(bundle) {
    ...
    const { arpGhostHsInfer, arpSynthOnsetHsSet } = arpAndSlidePrepasses.computeArpGhostHsInfer(bundle.handShapes, bundle.chordTemplates, notes);

    // Both feed singleNoteRenderer.drawSingleNotes() below.
    const _arpPersistKeys = arpAndSlidePrepasses.computeArpPersistKeys(arpGhostHsInfer, bundle.handShapes, notes, now, t0);
    ({
      slideTargetSet: _slideTargetSet,
      slideTargetNotesRef: _slideTargetNotesRef,
      slideTargetChordsRef: _slideTargetChordsRef,
    } = arpAndSlidePrepasses.computeSlideTargetSet(notes, bundle.chords, _slideTargetSet, _slideTargetNotesRef, _slideTargetChordsRef));

    const { laneRailArpHsFlags, laneRailBoundLo, laneRailBoundHi } = arpAndSlidePrepasses.computeLaneRailCaches(bundle.handShapes, chords, bundle.chordTemplates, notes || []);
    const { beats } = bundle;
    ...
    _scrEventTimesLen = lookaheadPrepasses.computeEventTimesUnion(_drawNextByString, _drawRecentByString, nStr, _scrEventTimes);
    ...
    if (cameraBootstrap.detectSongChangeAndResetCamera(bundle)) {
      ...
      _clkAudioT = NaN; _clkPerf = NaN; _clkRate = 1; _clkOutputT = NaN;
    }
    cameraBootstrap.bootstrapCameraFromChartData(
      ...
      now,
      nStr,
      ctx.settings.cameraMode,
      lookaheadBoundsNow,
      camAhead,
      camTau,
      camHystF,
      camDistHystF,
      ctx.settings.cameraLockLow,
      ctx.settings.cameraLockZoom,
    );
    ...
    const {
      highwayIntensity, camWX, camWSum, camDistMin, camDistMax, camDistGot,
    } = _chordAccum;
    ...
    const { hwyLaneFretClipMin, hwyLaneFretClipMax } = highwayLane.drawHighwayLane(
      anchors,
      bundle,
      now,
      chords,
      ...
      laneRailBoundHi,
      highwayIntensity,
      ctx.settings.fretDividersVisible,
    );
    ...
    fretColumnMarkers.drawFretColumnMarkers(
      ...
      chords,
      anchors,
      ctx.settings.fretColumnMarkerCadence,
      nStr,
      _textSizeMul,
      hwyLaneFretClipMin,
      hwyLaneFretClipMax,
  ...
  function teardown() {
    ...
    mLaneOdd = mLaneEven = mLaneDivider = mLaneDividerArp = null;
    ...
    _measureStarts = []; _measureStartsRef = null;
    _clkAudioT = NaN; _clkPerf = NaN; _clkRate = 1; _clkOutputT = NaN; _clkOutputPerf = NaN; _frameNow = 0;
  ...
  return {
    ...
    draw(bundle) {
      ...
      if (bgGroup && backgroundMount.effectiveBackgroundStyleId() !== 'off') {
        ...
        if (style && bgState) {
          try { style.update(bgState, bands, dt, nowMs / 1000); } catch (e) { console.error('[3D-Hwy] bg update threw', backgroundMount.effectiveBackgroundStyleId(), e); }
        }
      }
      ...
      if (bcCtrl) {
        ...
        if (cfg.colorTint && butterchurnTintTarget != null) {
          const tr = (butterchurnTintTarget >> 16) & 255; const tg = (butterchurnTintTarget >> 8) & 255; const
            tb = butterchurnTintTarget & 255;
      ...
      if (lyricsCtx && lyricsCanvas) {
        ...
        if (ctx.settings.sectionHudVisible && bundle.sections && bundle.sections.length) {
          const secH = hudsCache.drawSectionHud(lyricsCtx, {
            sections: bundle.sections,
            currentTime: bundle.currentTime,
            canvasW: lyricsCanvas.width,
            canvasH: lyricsCanvas.height,
        ...
        if (ctx.settings.toneHudVisible && (bundle.toneChanges?.length || bundle.toneBase)) {
          const toneH = hudsCache.drawToneHud(lyricsCtx, {
        ...
        if (ctx.settings.chordDiagramVisible && _diagPrev && _diagPrevOpacity > 0) {
          chordDiagramCache.drawDiagramCached(lyricsCtx, {
            ...
            sizeSlider: ctx.settings.chordDiagramSize,
            position: ctx.settings.chordDiagramPosition,
        ...
        if (ctx.settings.chordDiagramVisible && _diagChord) {
          const diagH = chordDiagramCache.drawDiagramCached(lyricsCtx, {
            name: _diagChord.name,
            frets: _diagChord.frets,
      ...
      if (lyricsCtx && lyricsCanvas
        ...
        window.highway.fireDrawHooks(lyricsCtx, lyricsCanvas.width, lyricsCanvas.height);
      }
    },
 ...
 */
window.feedBackViz_highway_3d_dev.contextType = 'webgl2';
window.feedBackViz_highway_3d_dev.__test = {
 ...
 */
window.feedBackViz_highway_3d_dev.matchesArrangement = function matchesArrangement(songInfo) {
...
// relevance on whether highway_3d_dev is the active viz. register() is still the right hook for a
