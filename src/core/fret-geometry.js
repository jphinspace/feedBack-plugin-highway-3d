import {
  FRET_SCALE, FRET_SPACING_ANCHOR_F, FRET_SPACING_STRETCH_ABOVE12, NFRETS, K, TS,
} from './constants.js';

const _fretXLog = (f) => {
  if (f <= 0) return 0;
  const raw = FRET_SCALE - FRET_SCALE / 2 ** (f / 12);
  if (f <= FRET_SPACING_ANCHOR_F) return raw;
  const rawAnchor = FRET_SCALE - FRET_SCALE / 2 ** (FRET_SPACING_ANCHOR_F / 12);
