...
export function _aspectWriteVal(k, v) {
  const base = _aspectTune();
  if (!_aspectEditTarget) { base[k] = v; } else {
...
export function _aspectClearVal(k) {
  const base = _aspectTune();
  if (!_aspectEditTarget) { base[k] = null; } else {
    const m = base.__panels; const
      ov = m && m[_aspectEditTarget];
    ...
    const tick = () => {
      ...
      if (_aspectPanelRO && ro) {
        ...
        if (e && Number.isFinite(e.aspect)) {
          _aspectPanelRO.textContent = `aspect ${e.aspect.toFixed(2)} · vFOV ${e.vfov.toFixed(1)}°`;
        }
      }
